...
@interface MM_EditExpendVC () <UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation MM_EditExpendVC
- (instancetype)initWithModel:(MM_AccountCellModel *)model change:(nonnull void (^)(MM_AccountCellModel * _Nonnull))change {
    if (self = [super init]) {
        self.oldModel = model;
        self.change = change;
    }return self;
...
- (void)viewDidLoad {
    ...
    self.keyboard = [MM_BillKeyboard date:^{
        ...
        MM_AccountCellModel *model = [[MM_AccountCellModel alloc]initWithImage:self.selectModel.highlightImage remark:remark price:price tag:self.selectModel.title rype:BillTypeExpend date:self.dateString];
